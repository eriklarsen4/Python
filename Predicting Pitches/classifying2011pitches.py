...
""" Exploratory analysis to determine predictions of pitch types over the 2011 season.
    Constrained by speed within ~ 15s: naive Bayes multi-class classifier and gradient-boosted machines.
    ...
    Yet, I surprisingly found this was extremely unimportant, and obviously computationally costly.
    
    I then cut all my features down and compared using gridsearch as cross-validation, optimizing one parameter
    ...
    gridsearch may not be viable pitch-to-pitch to re-fit, or even between innings, but if the model could be built offline
    and tested for games each day, it might be more informative.
    
    I measured accuracy with a confusion matrix, precision, recall and F1.
    Naive Bayes performed faster at roughly same or slightly better than gridsearched GBM. """
